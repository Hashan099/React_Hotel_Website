...
const Header = () => {
  ...
  const handleButtonClick = () => {
    alert('please call us +94-81-256-524-7 or send an email');
  };
      ...
      <div className="flex justify-between items-center p-4 pt-8 container m-auto">
        <div className={`hidden md:block ${isMenu ? "block" : "hidden"}`}>
          <ul className="list-none capitalize flex">
            ...
            <Link to={"/Category"}><li className="px-4 relative cursor-pointer">Categories</li></Link>
        ...
        <div className="hidden md:block">
          <button className="px-4 p-3 uppercase font-normal">
            <DiamondIcon/>
            <span className="p-1">For Reservations</span>
          </button>
          <button onClick={handleButtonClick} className="px-4 p-3 bg-[#d27548] uppercase text-white font-normal rounded-full">
          Inquiry 
          </button>
        </div>
